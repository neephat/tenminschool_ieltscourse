...
import RightColumnStickySetup from "@/components/RightColumnStickySetup";
...
export async function getCourseData(lang: string | undefined) {
  const language = (lang || "BN").toLowerCase();
...
export default async function Home({ searchParams }: PageProps) {
  const lang = searchParams.lang?.toUpperCase() || "BN";
  const data = await getCourseData(lang);
  const course = data.course;
  const sections = data.sections;

  const layout = sections.find(
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-14 pb-12 pt-5">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative ">
          ...
          <div className="lg:col-span-2 space-y-8 ">
            ...
            <div id="layout" className="scroll-mt-24">
              <CourseLayout data={layout} />
            </div>
            ...
            <div id="about" className="scroll-mt-24">
              <CourseDetailsAbout about={about} />
            </div>
